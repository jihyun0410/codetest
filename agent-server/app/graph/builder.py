...
정의서 "(1) workflow 생성":
  ...
  · workflow 생성은 LLM 토큰 소비 없이 AST 파싱(Tree-sitter)만으로 진행한다
"""
...
from app.parsing.registry import detect_language, parse_source
...
class GraphBuilder:
    ...
    def _locate_build_files(self, filename: str, max_matches: int = 5) -> list:
        ...
        from app.parsing.registry import EXCLUDED_DIRS
