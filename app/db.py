...
class Workflow(Base, IdMixin, TimestampMixin):
    """진입점에서 Calls/Executes 를 따라 추적한 기능(Flow) 단위 실행 흐름."""

    __tablename__ = "workflows"
    __table_args__ = (UniqueConstraint("project_id", "key", name="uq_workflow_key"),)

    project_id: Mapped[str] = mapped_column(
        ForeignKey("projects.id", ondelete="CASCADE"), nullable=False, index=True
    )
    #: 진입점 노드의 qualified_name — 재생성 시 동일 Flow 판별 키
    key: Mapped[str] = mapped_column(String(1000), nullable=False)
    #: 사람이 읽는 Flow 이름 (예: "POST /api/users/login")
    name: Mapped[str] = mapped_column(String(300), nullable=False)
    entry_node_id: Mapped[str | None] = mapped_column(String(32), nullable=True)
    #: [{step_id, node_id, node_type, label, physical_path, logical_path, line, kind}, ...]
    steps: Mapped[list] = mapped_column(JSON, default=list)
    #: [{source: step_id, target: step_id, edge_type}, ...]
    transitions: Mapped[list] = mapped_column(JSON, default=list)
    #: 이 Flow 가 관여하는 파일 목록
    involved_files: Mapped[list] = mapped_column(JSON, default=list)
    last_change: Mapped[str | None] = mapped_column(String(20), nullable=True)
